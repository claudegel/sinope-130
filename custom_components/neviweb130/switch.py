...
SWITCH_TYPES = {
    ...
    "control": ["mdi:alarm", BinarySensorDeviceClass.PROBLEM],
}
...
async def async_setup_platform(
        ...
        if "signature" in device_info and \
            ...
            if device_info["signature"]["model"] in IMPLEMENTED_WALL_DEVICES:
              ...
              or device_info["signature"]["model"] in IMPLEMENTED_ZB_DEVICE_CONTROL:
                ...
                device_type = "valve"
                entities.append(Neviweb130ValveSwitch(data, device_info, device_name, device_sku, device_firmware, device_type))
            elif device_info["signature"]["model"] in IMPLEMENTED_WIFI_VALVE_MODEL:
                device_type = "valve"
                entities.append(Neviweb130WifiValveSwitch(data, device_info, device_name, device_sku, device_firmware, device_type))
            elif device_info["signature"]["model"] in IMPLEMENTED_ZB_MESH_VALVE_MODEL:
                device_type = "flow"
                entities.append(Neviweb130MeshValveSwitch(data, device_info, device_name, device_sku, device_firmware, device_type))
            else:
                device_type = "flow"
                entities.append(Neviweb130WifiMeshValveSwitch(data, device_info, device_name, device_sku, device_firmware, device_type))
    ...
    def set_input_output_names_service(service):
        ...
        for switch in entities:
            if switch.entity_id == entity_id:
                value = {"id": switch.unique_id, "input1": service.data[ATTR_NAME_1], "input2": service.data[ATTR_NAME_2], "output1": service.data[ATTR_OUTPUT_NAME_1], "output2": service.data[ATTR_OUTPUT_NAME_2]}
...
class Neviweb130ValveSwitch(Neviweb130Switch):
    """Implementation of a Neviweb valve switch, VA4200ZB."""
...
class Neviweb130WifiValveSwitch(Neviweb130Switch):
    ...
    def update(self):
        if self._activ:
            LOAD_ATTRIBUTES = [ATTR_WIFI, ATTR_MOTOR_POS, ATTR_MOTOR_TARGET, ATTR_TEMP_ALARM, ATTR_VALVE_INFO, ATTR_BATTERY_VOLTAGE, ATTR_BATTERY_STATUS, ATTR_POWER_SUPPLY, ATTR_VALVE_CLOSURE, ATTR_BATT_ALERT, ATTR_STM8_ERROR, ATTR_FLOW_METER_CONFIG, ATTR_FLOW_ALARM1, ATTR_FLOW_ALARM2, ATTR_TEMP_ACTION_LOW, ATTR_BATT_ACTION_LOW,
                            ATTR_OCCUPANCY_SENSOR_DELAY, ATTR_BATT_STATUS_NORMAL, ATTR_BATT_PERCENT_NORMAL, ATTR_WATER_LEAK_STATUS, ATTR_AWAY_ACTION]
            """Get the latest data from Neviweb and update the state."""
            start = time.time()
            device_data = self._client.get_device_attributes(self._id, UPDATE_ATTRIBUTES + LOAD_ATTRIBUTES)
            end = time.time()
