                ...
                if "signature" in device_info and "model" in device_info["signature"]:
                    ...
                    if model in IMPLEMENTED_DEVICE_MODEL:
                        ...
                        if device_info["signature"]["model"] in IMPLEMENTED_WALL_DEVICES:
                            ...
                            device_type = "power"
                            device = Neviweb130WifiPowerSwitch(data, device_info, device_name, device_sku, device_firmware, device_type)
                    ...
                    if model in IMPLEMENTED_ZB_DEVICE_CONTROL:
                        ...
                        for attribute in attributes_name:
                            ...
                            entities.append(Neviweb130DeviceAttributeSwitch(data['neviweb130_client'], device_info, device_name, attribute, device_entry.id, device_type))
...
class Neviweb130PowerSwitch(Neviweb130Switch):
    ...
    async def async_update(self):
        if self._activ:
            LOAD_ATTRIBUTES = [ATTR_WATTAGE, ATTR_WATTAGE_INSTANT, ATTR_TIMER, ATTR_KEYPAD, ATTR_DRSTATUS, ATTR_RSSI, ATTR_CONTROLLED_DEVICE, ATTR_ERROR_CODE_SET1]
            """Get the latest data from Neviweb and update the state."""
            start = time.time()
            device_data = await self._client.async_get_device_attributes(self._id, UPDATE_ATTRIBUTES + LOAD_ATTRIBUTES)
            end = time.time()
            ...
            if "error" not in device_data:
                if "errorCode" not in device_data:
                    ...
                    if device_data[ATTR_WATER_LEAK_STATUS] == "probe":
                        await self.async_notify_ha(
                            f"Warning: Neviweb Device error code detected: " + device_data[ATTR_WATER_LEAK_STATUS] + " for device: " + self._name + ", Sku: " + self._sku + ", Leak sensor disconnected"
                        )
                else:
                    _LOGGER.warning("Error in reading device %s: (%s)", self._name, device_data)
            else:
...
class Neviweb130ControlerSwitch(Neviweb130Switch):
    ...
    async def async_update(self):
        if self._activ:
            ...
            """Get the latest data from Neviweb and update the state."""
            start = time.time()
            device_data = await self._client.async_get_device_attributes(self._id, UPDATE_ATTRIBUTES + LOAD_ATTRIBUTES + NAME_ATTRIBUTES)
            end = time.time()
            elapsed = round(end - start, 3)
            _LOGGER.debug("Updating %s (%s sec): %s", self._name, elapsed, device_data)
...
class Neviweb130DeviceAttributeSwitch(SwitchEntity):
    ...
    def __init__(self, client: Neviweb130Client, device: dict, device_name: str, attribute: str, device_id: str, device_type: str):
...
#        self._device_name = device_name
        ...
        self._switch_name = f"{device_name} {attribute.replace('_', ' ').capitalize()}"
