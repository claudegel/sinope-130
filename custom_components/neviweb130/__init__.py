...
async def async_setup_entry(hass, entry):
    ...
    data = Neviweb130Data(entry.data)
...
class Neviweb130Data:
    ...
    def __init__(self, config):
        """Initialize the neviweb130 data object."""
        _LOGGER.debug("Entry = %s", config)
        self.username = config.get(CONF_USERNAME)
        self.password = config.get(CONF_PASSWORD)
        self.network = config.get(CONF_NETWORK)
        self.network2 = config.get(CONF_NETWORK2)
        self.network3 = config.get(CONF_NETWORK3)
        self.neviweb130_client = Neviweb130Client(self.username, self.password, self.network, self.network2, self.network3)

    async def async_initialize(self):
        """Initialize the Neviweb130 client asynchronously."""
        await self.neviweb130_client.async_initialize()
        _LOGGER.debug("neviweb130_client = %s", self.neviweb130_client)
...
class Neviweb130Client:
    ...
    async def async_update(self):
        await self._async_get_gateway_data()

    async def async_reconnect(self):
        await self._async_post_login_page()
        await self._async_get_network()
        await self._async_get_gateway_data()
    ...
    async def async_post_login_page(self):
        ...
        try:
            async with session.post(LOGIN_URL, json=data, 
...
#                _LOGGER.debug("login status: %s", response.status)
    ...
    async def async_get_network(self):
        ...
        session = await create_session()
        try:
            async with session.get(LOCATIONS_URL + self._account, headers=self._headers, cookies=self._cookies, timeout=self._timeout) as response:
                raw_res = await response.json()
                networks = raw_res
                    ...
                    for network in networks:
                        ...
                        if self._network_name2 is not None and self._network_name2 != "":
                            if network["name"] == self._network_name2:
                                ...
                                self._gateway_id = network["id"]
                                _LOGGER.debug("Please check first letter of your network2 name, In capital letter or not? Selecting %s network among: %s", self._network_name2, networks)
                                continue
                            else:
                                _LOGGER.debug("Your network name %s do not correspond to discovered network %s, skipping this one...", self._network_name2, network["name"])
    ...
    async def async_get_gateway_data(self):
        ...
        if self._gateway_id == None and self._gateway_id2 == None and self._gateway_id3 == None:
            ...
            session = await create_session()
            try:
                async with session.get(GATEWAY_DEVICE_URL + str(self._gateway_id), headers=self._headers, cookies=self._cookies, timeout=self._timeout) as response:
                    raw_res = await response.json()
            ...
            if self._gateway_id2 is not None:
                try:
                    async with session.get(GATEWAY_DEVICE_URL + str(self._gateway_id2), headers=self._headers, cookies=self._cookies, timeout=self._timeout) as response:
            ...
            if self._gateway_id3 is not None:
                try:
                    async with session.get(GATEWAY_DEVICE_URL + str(self._gateway_id3), headers=self._headers, cookies=self._cookies, timeout=self._timeout) as response:
...
#            _LOGGER.debug("Updated gateway data3: %s", self.gateway_data3)
    ...
    async def async_get_device_attributes(self, device_id, attributes):
        ...
        session = await create_session()
        try:
            async with session.get(DEVICE_DATA_URL + str(device_id) + "/attribute?attributes=" + ",".join(attributes), 
    ...
    async def async_get_device_status(self, device_id):
        ...
        session = await create_session()
        try:
            async with session.get(DEVICE_DATA_URL + str(device_id) + "/status", headers=self._headers, cookies=self._cookies, timeout=self._timeout) as response:
                raw_res = await response.json()
    ...
    async def async_get_neviweb_status(self, location):
        ...
        session = await create_session()
        try:
            async with session.get(NEVIWEB_LOCATION + str(location) + "/notifications", headers=self._headers, cookies=self._cookies, timeout=self._timeout) as response:
                raw_res = await response.json()
    ...
    async def async_get_device_alert(self, device_id):
        ...
        session = await create_session()
        try:
            async with session.get(DEVICE_DATA_URL + str(device_id) + "/alert", headers=self._headers, cookies=self._cookies, timeout=self._timeout) as response:
                raw_res = await response.json()
                _LOGGER.debug("Received devices alert (%s): %s",str(device_id), raw_res) 
        except aiohttp.ClientError as e:
    ...
    async def async_get_device_monthly_stats(self, device_id):
        ...
        session = await create_session()
        try:
            async with session.get(DEVICE_DATA_URL + str(device_id) + "/energy/monthly", headers=self._headers, cookies=self._cookies, timeout=self._timeout) as response:
                raw_res = await response.json()
    ...
    async def async_get_device_daily_stats(self, device_id):
        ...
        session = await create_session()
        try:
            async with session.get(DEVICE_DATA_URL + str(device_id) + "/energy/daily", headers=self._headers, cookies=self._cookies, timeout=self._timeout) as response:
                raw_res = await response.json()
    ...
    async def async_get_device_hourly_stats(self, device_id):
        ...
        session = await create_session()
        try:
            async with session.get(DEVICE_DATA_URL + str(device_id) + "/energy/hourly", headers=self._headers, cookies=self._cookies, timeout=self._timeout) as response:
                raw_res = await response.json()
    ...
    async def async_get_device_sensor_error(self, device_id):
        ...
        session = await create_session()
        try:
            async with session.get(DEVICE_DATA_URL + str(device_id) + "/attribute?attributes=errorCodeSet1", headers=self._headers, cookies=self._cookies, timeout=self._timeout) as response:
                raw_res = await response.json()
    ...
    async def async_set_aux_cycle_output(self, device_id, status, val):
    ...
    async def async_set_led_indicator(self, device_id, state, intensity, red, green, blue):
        ...
        if state == 1:
            ...
            await self.async_set_device_attributes(device_id, data2)
            _LOGGER.debug("led.data on = %s, led.data2 = %s", data, data2)
        else:
            data = {ATTR_LED_OFF_COLOR:{"red":red,"green":green,"blue":blue}}
            await self.async_set_device_attributes(device_id, data)
            data2 = {ATTR_LED_OFF_INTENSITY:intensity}
            await self.async_set_device_attributes(device_id, data2)
            _LOGGER.debug("led.data off = %s, led.data2 = %s", data, data2)
    ...
    async def async_set_device_attributes(self, device_id, data):
        ...
        while result < 3:
            session = await create_session() 
            try:
                async with session.put(DEVICE_DATA_URL + str(device_id) + "/attribute", json=data, headers=self._headers, cookies=self._cookies, timeout=self._timeout) as response:
                    resp = await response.json()
                    _LOGGER.debug("http_request = %s%s%s %s", DEVICE_DATA_URL, str(device_id), "/attribute", data)
                ...
                if "error" in resp:
                    result += 1
                    _LOGGER.debug("Service error received: %s, resending request %s",resp, result)
                    continue
                else:
                    break
